...
final class MatchDemoConfigBootstrap {

    private MatchDemoConfigBootstrap() {
    }
    ...
    static ConfigManager bootstrap(Path dataDirectory, ClassLoader classLoader, Consumer<String> logger) {
        ...
        logger.accept("[demo-config] plugin=match, sample-text=" + demoConfig.sampleText() + ", sample-number=" + demoConfig.sampleNumber() + ", auto-flag=" + demoConfig.autoFlag());
        logger.accept("[demo-messages] plugin=match, greeting=" + demoMessages.greeting() + ", footer=" + demoMessages.footer());
