...
final class ProxyDemoConfigBootstrap {

    private ProxyDemoConfigBootstrap() {
    }
    ...
    static ConfigManager bootstrap(Path dataDirectory, ClassLoader classLoader, Consumer<String> logger) {
        ...
        logger.accept("[demo-config] plugin=proxy, sample-text=" + demoConfig.sampleText() + ", sample-number=" + demoConfig.sampleNumber() + ", auto-flag=" + demoConfig.autoFlag());
        logger.accept("[demo-messages] plugin=proxy, greeting=" + demoMessages.greeting() + ", footer=" + demoMessages.footer());
