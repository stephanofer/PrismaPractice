...
final class FfaDemoConfigBootstrap {

    private FfaDemoConfigBootstrap() {
    }
    ...
    static ConfigManager bootstrap(Path dataDirectory, ClassLoader classLoader, Consumer<String> logger) {
        ...
        logger.accept("[demo-config] plugin=ffa, sample-text=" + demoConfig.sampleText() + ", sample-number=" + demoConfig.sampleNumber() + ", auto-flag=" + demoConfig.autoFlag());
        logger.accept("[demo-messages] plugin=ffa, greeting=" + demoMessages.greeting() + ", footer=" + demoMessages.footer());
